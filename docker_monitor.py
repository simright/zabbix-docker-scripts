...
def check_container_stats(container_name,collect_item):
    ...
    elif collect_item == 'mem_sys_usage': # total memory usage of system 
        result=new_result['memory_stats']['usage']
    elif collect_item == 'mem_docker_usage':       # memory usage of container
        result=new_result['memory_stats']['stats']['total_rss']
    elif collect_item == 'mem_limit':       # total memory of system
        result=new_result['memory_stats']['limit']
    elif collect_item == 'mem_sys_percent': # memory usage percent of system
        mem_usage=new_result['memory_stats']['usage']
        mem_limit=new_result['memory_stats']['limit']
        result=round(float(mem_usage)/float(mem_limit)*100.0,2)
    elif collect_item == 'mem_docker_percent':  # memory usage percent of docker container
        mem_usage=new_result['memory_stats']['']
        mem_limit=new_result['memory_stats']['limit']
        result=round(float(mem_usage)/float(mem_limit)*100.0,2)
